...
const products = [
  ...
  },
  {
    ...
    calculation: {
      individualTotal: 33.05,
      kitPrice: 26,
      discount: 7.05,
      note: "Comprando o kit completo, você economiza R$ 7,05 em comparação com os itens avulsos."
    }
  }
];
...
function renderProducts(category = "Todos") {
  ...
  const catalogProducts = products.filter((product) => !product.hiddenFromCatalog);

  if (category === "Todos") {
    productGrid.classList.add("is-grouped");
    productGrid.innerHTML = categoryOrder.map((groupName) => {
...
function buildWhatsAppUrl(customerName) {
  const message = `Olá, Apex 3D! Acabei de fazer um pedido no site em nome de ${customerName} e quero finalizar o pagamento por Pix.`;
